...
function getCellsLow(x0,y0, x1,y1) {
    ...
    for (var x = x0; x <= x1; x++) { // change <= or < here to change inclusive/exclusive
...
function getCellsHigh(x0,y0, x1,y1) {
    ...
    for (var y = y0; y <= y1; y++) { // change <= or < here to change inclusive/exclusive
        toReturn.push({"x": x, "y": y});
        if (D > 0) {
            x = x + xi;
            D = D - 2*dy;
...
// Gets INCLUSIVE cells; for EXCLUSIVE of end-point, look up comments above
